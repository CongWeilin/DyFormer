...
class DySAT(nn.Module):
    ...
    def forward(
        ...
        for structural_layer in self.structural_attention_layer_list:
            structural_att_outputs = []
            for t in range(T):
                # input: [Nt, D] -> [Nt, F/H]; after concat: [Nt, F]
                x = structural_layer(structural_inputs[t], adjs[t], edges[t])
                structural_att_outputs.append(x)
            structural_inputs = structural_att_outputs
        ...
        for t in range(T):
            zero_padding = torch.zeros(
                size=(structural_att_outputs[-1].shape[0] - structural_att_outputs[t].shape[0], 
                      self.num_structural_hids_list[-1]),  # this is for N_T - N_t compensation
                dtype=torch.float32,
                device=device,
            )
        ...
        for temporal_layer in self.temporal_attention_layer_list:
            outputs = temporal_layer(temporal_inputs, device)
...
class StructuralAttentionLayer(nn.Module):
    ...
    def __init__(
        ...
        self.edge_W = nn.Parameter(torch.Tensor(self.edge_dim, 128))
        ...
        self.a_list = nn.ParameterList([
            nn.Parameter(torch.Tensor(1, self.output_dim * 2 // self.n_heads + 128)) 
    ...
    def forward(
        ...
        edge_feat,                      # [E, DE]: edge feature matrix
    ):
        """
        ...
        for head in range(self.n_heads):
            att_ = self._single_head_attn(x, adj_mat,
                                          w=self.W_list[head],
                                          a=self.a_list[head],
                                          e=edge_feat)
            # print(x, att_)
    ...
    def _single_head_attn(self, x, adj_mat, w, a, e):
        ...
        if torch.isnan(h_prime).any():
            h_prime = torch.nan_to_num(h_prime, nan=0.)
        return h_prime
...
class TemporalAttentionLayer(nn.Module):
    ...
    def forward(
        ...
        position_inputs = torch.arange(T, device=device)
        position_inputs = position_inputs.expand(N, T)  # [N, T]: 
