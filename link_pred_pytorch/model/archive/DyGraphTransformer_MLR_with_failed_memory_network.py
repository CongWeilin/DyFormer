...
class DyGraphTransformer(nn.Module):
    ...
    def forward(self, x, node_encodes, edge_encodes, edge_dist_encodes, device):
...
class MemoryLowRankAttention(nn.Module):
    def __init__(self, hidden_size, head_size, memory_size, attention_dropout_rate, use_memory_net):
        ...
        if self.use_memory_net: # use explicit write header to update memory
            memory = torch.Tensor(1, memory_size, hidden_size)   
            self.register_buffer('memory', memory.data) # create self.memory that not be update by GD
        else:                   # use gradient descent to update memory
    ...
    def forward(self, target_x, context_x, attn_bias=None):
        ...
        memory_norm = self.memory.norm(p=2, dim=1, keepdim=True) 
        self.memory = self.memory.div(memory_norm)
        ...
        v = self.linear_v(context_x).view(batch_size, -1, self.head_size, d_v)
        ...
        v = self.linear_v(augmented_memory).view(batch_size, -1, self.head_size, d_v)
        q = q.transpose(1, 2)                  # [b, h, q_len, d_k]
        k = k.transpose(1, 2).transpose(2, 3)  # [b, h, d_k, k_len]
        v = v.transpose(1, 2)                  # [b, h, v_len, d_v]
